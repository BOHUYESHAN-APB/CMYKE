...
- 目标是形成类似 Neuro-sama 3D 直播中的“主动运动感”：持续、目标导向、可被打断。
- 动作决策优先来自 Boss/Manager/Spine 的模型策略；硬编码规则只保留在安全闸门、限幅、急停与降级。
- 评估不以“脚本触发次数”衡量，而以目标完成率、无效交互率、碰撞率、人工接管率等指标衡量。
...
- 验收补充：在非脚本任务中保持连续主动运动，规则层不主导动作生成。
