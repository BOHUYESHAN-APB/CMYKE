...
- Local realtime voice implementation remains an active workstream (not archived).
- VLA/embodiment optimization under `Studying/` also remains active for future product integration.

## Local Tracking Policy
- This handoff is for local AI/agent continuity.
- Keep commits local unless explicitly approved to sync to remote/cloud.

## Stage Goals (Current)
- Continue local speech-to-speech optimization (latency, barge-in stability, XPU/CPU fallback).
...
- Continue VLA sandbox optimization for proactive 3D movement with fewer scripted condition reactions.
...
## Program Correction (2026-02-26)
- Repo root `HANDOFF.md` (VLA sandbox) and `docs/HANDOFF.md` (product/runtime handoff) are both active.
- Local voice and VLA are parallel tracks; neither is deferred.
- Embodiment target is model-driven proactive movement (Neuro-sama-like agency), with hard rules reserved for safety/fallback.
- Promotion path stays: `Studying/` experiment -> protocol freeze -> backend/client production integration.
...
- Repo cleanliness should be checked live (`git status -sb`), not assumed from this snapshot.
